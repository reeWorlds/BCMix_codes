...
uint64_t _numbers[Tsize], extra_numbers[Tsize];
uint8_t _n[Tsize];
uint16_t _transfer[Tsize], _mult[Tsize];

uint32_t long_flag[Tsize];
...
void form_ter_table_long()
{
	...
	for (uint32_t y = 0; y < Tsize; y++)
	{
		...
		_numbers[y] = _n[y] = _transfer[y] = extra_numbers[y] = 0;
		_flag[y] = 0xffff; long_flag[y] = 0xffffff;

		for (int i = 0; i < Tblock_size; i += 2, k = k_prev ? k : (k ? k * 3 : 1), x >>= 2)
		{
			if ((x & 0x3) == 3)
			{
				if (numb <= 1)
				{
					_numbers[y] |= (p << (numb * 32));
				}
				else
				{
					extra_numbers[y] |= (p << ((numb - 2) * 32));
				}
				numb++;
				p = 0;
...
void decode_ternary_long(uint8_t* file, uint32_t fileN, uint32_t*& codes, uint32_t& codesN)
{
	...
	for (int32_t i = 0; i < fileN; i += 8)
	{
		...
		for (int32_t j = 0; j < loop_n; j++, t >>= Tblock_size)
		{
			x = t & Tmask;
			z = tr * _mult[x];
			*(uint64_t*)(codes + k) = _numbers[x] + z;
			if (_n[x] > 2)
			{
				*(uint64_t*)(codes + k + 2) = extra_numbers[x];
			}
			k += _n[x];
		...
		z = tr * _mult4[t];
		*(uint64_t*)(codes + k) = _numbers4_long[t] + z;
		k += _n[t];
		tr = _transfer4[t] + (z & long_flag[t]);
	}
...
int main(int argc, char** argv)
{
	...
	cout << "Start BC3_long_6blocks " << argv[2] << "\n";
	...
	for (int32_t iter = 0; iter < iters; iter++)
	{
		...
		decode_ternary_long(d_file, d_fileN, d_codes, d_codesN);
